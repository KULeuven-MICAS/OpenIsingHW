from dimc_validation import (
    ...
    dimc_cost_estimation,
    dimc_ISSCC2022_11_7,
    dimc_ISSCC2022_15_5,
    dimc_ISSCC2023_7_2,
)
from dimc_validation4 import cacti_value_ISSCC2023_16_3, dimc_ISSCC2023_16_3
...
def area_fitting():
    ...
    for area in range(294, 1000, 1):
        ...
        _dt = (d1 + d2 + d3 + d4) / 4  # average delay mismatch
        _et = (e1 + e3) / 2  # average energy mismatch (peak energy is not reported in paper2)
        if at < mismatch:
            mismatch = at
...
def delay_fitting():
    ...
    for delay in range(150, 500, 1):
        ...
        a3, d3, e3 = dimc_cost_estimation(dimc_ISSCC2023_7_2, cacti_value_ISSCC2023_7_2)
        a4, d4, e4 = dimc_cost_estimation4(dimc_ISSCC2023_16_3, cacti_value_ISSCC2023_16_3)
        _at = (a1 + a2 + a3 + a4) / 4  # average area mismatch
...
def cap_fitting():
    ...
    for cap in range(1, 50, 1):
        ...
        a2, d2, e2 = dimc_cost_estimation(dimc_ISSCC2022_11_7, cacti_ISSCC2022_11_7)
        a3, d3, e3 = dimc_cost_estimation(dimc_ISSCC2023_7_2, cacti_value_ISSCC2023_7_2)
        _at = (a1 + a2 + a3) / 3  # average area mismatch
        _dt = (d1 + d2 + d3) / 3  # average delay mismatch
