...
from dimc_cost_model import Adder, AdderTree, MemoryInstance, MultiplierArray, UnitDff, UnitNand2
...
def aimc2_cost_estimation(aimc, cacti_value):
    ...
    _reg_input_bitwidth = aimc["reg_input_bitwidth"]
    _input_bandwidth = input_channel * aimc["input_precision"]
    _output_bandwidth_per_channel = aimc["output_precision"]
    """
    ...
    adder1 = Adder(
        vdd=aimc["vdd"], input_precision=7, unit_area=unit_area, unit_delay=unit_delay, unit_cap=unit_cap
    )  # 8 in total
    adder2 = Adder(
        vdd=aimc["vdd"], input_precision=9, unit_area=unit_area, unit_delay=unit_delay, unit_cap=unit_cap
    )  # 4 in total
    adder3 = Adder(
        vdd=aimc["vdd"], input_precision=12, unit_area=unit_area, unit_delay=unit_delay, unit_cap=unit_cap
    )  # 2 in total
    adder4 = Adder(
        vdd=aimc["vdd"], input_precision=15, unit_area=unit_area, unit_delay=unit_delay, unit_cap=unit_cap
    )  # 1 in total
    ...
    _predicted_tops = number_of_operations / (predicted_delay * number_of_cycle) / (10**3)
    _predicted_topsw = number_of_operations / predicted_energy * 10**3

    ## Energy breakdown per MAC
    number_of_mac = number_of_operations / 2
    _energy_mults_mac = energy_mults * number_of_cycle / number_of_mac
    _energy_adder_tree_mac = energy_adder_tree * number_of_cycle / number_of_mac
    _energy_accumulator_mac = energy_accumulator * number_of_cycle / number_of_mac
    _energy_banks_mac = energy_banks * number_of_cycle / number_of_mac
    _energy_regs_accumulator_mac = energy_regs_accumulator * number_of_cycle / number_of_mac
    _energy_regs_pipeline_mac = energy_regs_pipeline * number_of_cycle / number_of_mac
    _energy_adc_mac = energy_adc * number_of_cycle / number_of_mac
    _energy_dac_mac = energy_dac * number_of_cycle / number_of_mac
    energy_estimation_per_mac = predicted_energy / number_of_mac
    energy_reported_per_mac = 2000 / aimc["TOP/s/W"]

    _area_mismatch = abs(predicted_area / aimc["area"] - 1)
    _delay_mismatch = abs(predicted_delay / aimc["tclk"] - 1)
    _energy_mismatch = abs(energy_estimation_per_mac / energy_reported_per_mac - 1)
