...
def dimc_cost_estimation(dimc, cacti_value):
    ...
    regs_output = MemoryInstance(
        ...
        delay=unit_reg.calculate_delay(),
        r_energy=0,
        w_energy=unit_reg.calculate_cap() * dimc["vdd"] ** 2 * output_bandwidth_per_channel,
    ...
    if dimc["pipeline"]:
        ...
        regs_pipeline = MemoryInstance(
            ...
            delay=unit_reg.calculate_delay(),
            r_energy=0,
            w_energy=unit_reg.calculate_cap() * dimc["vdd"] ** 2 * pipeline_bw_per_channel,
    ...
    if dimc["paper_idx"] == "ISSCC2022, 15.5":  # extra area cost for supporting FP operation
        ...
        extra_accumulator = Adder(
            vdd=dimc["vdd"], input_precision=32, unit_area=unit_area, unit_delay=unit_delay, unit_cap=unit_cap
        )
    ...
    if dimc["pipeline"]:
        if dimc["paper_idx"] == "ISSCC2022, 11.7":  # for dimc2
            ...
            predicted_delay = max(
                unitbank.delay + mults.calculate_delay(), adder_tree.calculate_delay() + accumulator_delay
            )
        else:
            # predicted_delay = max(unitbank.delay + mults.calculate_delay() + adder_tree.calculate_delay(), accumulator.calculate_delay_msb())
            predicted_delay = max(
    ...
    _energy_regs_input = dimc["banks"] * regs_input.w_energy
    _energy_regs_output = dimc["banks"] * dimc["output_channel"] * regs_output.w_energy
    ...
    if dimc["paper_idx"] == "ISSCC2023, 7.2":
        ...
        _energy_regs_output = dimc["banks"] * regs_output.w_energy
    ...
    if dimc["paper_idx"] == "ISSCC2023, 7.2":
        number_of_operations = (
            2 * dimc["banks"] * dimc["output_channel"] * dimc["input_channel"] / dimc["weight_precision"]
        )  # 1MAC = 2 Operations
