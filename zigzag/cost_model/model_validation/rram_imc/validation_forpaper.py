...
metrics_data_list = [
    {
        ...
        'comparisons': [
            {'name': '1T1R', 'user_breakdown': {'Cells': 54.25624615384615, 'WL Drivers': 43.25606484072191,
                                                'BL Drivers': 0.7632468591132585, 'DACs': 0, 'ADCs': 20.74927104,
                                                'PV Adders': 1.306367996, 'Accumulators': 0.8164799},
             'paper_reported_total': 106.1139896},
            {'name': '2T2R PC', 'user_breakdown': {'Cells': 138.52800319487997, 'WL Drivers': 583.9568753497457,
                                                   'BL Drivers': 1.7090084204787814, 'DACs': 10.368000000000002,
                                                   'ADCs': 44.869386240000004, 'PV Adders': 0.0, 'Accumulators': 0},
        ...
        'comparisons': [
            ...
            {'name': '2T2R PC',
             'user_breakdown': {'Cells': 0, 'WL Drivers': 0, 'BL Drivers': 0, 'DACs': 0, 'ADCs': 23.076923076923073,
                                'PV Adders': 0, 'Accumulators': 0}, 'paper_reported_total': 23.08},
        ]
    },
    {
        'title': 'Area',
        'unit': 'Norm. area (%)',
        'comparisons': [
            {'name': '1T1R', 'user_breakdown': {'TXs': 0.411041792, 'DACs': 0, 'ADCs': 0.0029284645108206254,
                                                'PV Adders': 0.0004597632, 'Accumulators': 0.021691391999999997,
                                                'WL Drivers': 0.16956377417562987, 'BL Drivers': 0.04255186044762985},
             'paper_reported_total': 0.75},
            {'name': '2T2R PC',
             'user_breakdown': {'TXs': 0.205520896, 'DACs': 0, 'ADCs': 0.13341969370468265, 'PV Adders': 0.0,
                                'Accumulators': 0, 'WL Drivers': 0.08478188708781494,
                                'BL Drivers': 0.02384945891162985}, 'paper_reported_total': 0.448},
        ]
    }
]
...
component_color_map = {name: component_colors_hex[i % len(component_colors_hex)] for i, name in
                       enumerate(sorted_unique_component_names)}
...
for i, metric_data in enumerate(metrics_data_list):
    ...
    for k, comp_set_data in enumerate(comparisons_list):
        ...
        p_bar = ax.bar(paper_bar_center_x, normalized_paper_total, bar_width_single, color=paper_total_bar_color,
        ...
        ax.text(paper_bar_center_x, normalized_paper_total / 2,
        ...
        for component_name, value in user_breakdown_dict.items():
            ...
            rect = ax.bar(user_bar_center_x, normalized_value, bar_width_single, bottom=current_bottom,
                          color=component_color_map.get(component_name, '#000000'),
                          label=component_name, edgecolor='white', linewidth=0.25)
        ...
        ax.text(label_x_pos, label_y_pos, f'{current_bottom:.0f}%',
...
if legend_handles_for_components:
    sorted_comp_handles = [legend_handles_for_components[name] for name in sorted_unique_component_names if
                           name in legend_handles_for_components]
    all_legend_handles.extend(sorted_comp_handles)

if all_legend_handles:
    fig.legend(handles=all_legend_handles, loc='lower center',
               bbox_to_anchor=(0.5, 0.81), ncol=4, fontsize=6,
