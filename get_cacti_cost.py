...
def get_cacti_cost(cacti_path, tech_node, mem_type, mem_size_in_byte, bw, hd_hash="a"):
    ...
    """
    # check if mem_size_in_byte < 4096B
    if mem_size_in_byte < 4096:
        mem_size_scaling_factor = mem_size_in_byte / 4096
        mem_size_in_byte_scaled = 4096  # set to minimum 4KB
        logging.info(f"mem_size_in_byte < 4096B ({mem_size_in_byte}B). Adjust to 4096B for CACTI input.")
    else:
        mem_size_in_byte_scaled = mem_size_in_byte
        mem_size_scaling_factor = 1

    # check if bw > 32
    if bw > 32:  # adjust the setting for CACTI
        rows = mem_size_in_byte_scaled * 8 / bw
        line_size = int(32 / 8)
        IO_bus_width = 32
        mem_size_in_byte_adjust = rows * 32 / 8
        logging.info(f"bw > 32 ({bw}bit). Adjust to 32bit for CACTI input. Mem size adjusted from {mem_size_in_byte_scaled}B to {mem_size_in_byte_adjust}B.")
    else:  # normal case
        rows = mem_size_in_byte_scaled * 8 / bw
        line_size = int(bw / 8)  # how many bytes on a row
        IO_bus_width = bw
        mem_size_in_byte_adjust = mem_size_in_byte_scaled
    ...
    C = CactiConfig()
    C.cacti_auto(['single', [['technology', 'cache_size', 'line_size', 'IO_bus_width', 'mem_type'],
    ...
    try:
        ...
        if mem_size_in_byte < 4096:
            area *= mem_size_scaling_factor
    except KeyError:
        logging.critical(f'**KeyError** in result, current result: {result}')
        breakpoint()
...
if __name__ == '__main__':
    # an example for use (28nm)
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s - %(levelname)s - %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S')
    for bw in [1024]:
        mem_size = 10*1024  # byte
        rows = mem_size * 8 / bw
        access_time, area, r_cost, w_cost = get_cacti_cost(cacti_path='./cacti/cacti_master', tech_node=0.028,
                                                           mem_type='sram', mem_size_in_byte=mem_size, bw=bw)
        print(
            f'access time (ns): {access_time}, area (mm2): {area}, r_cost (pJ): {r_cost}, w_cost (pJ): {w_cost}, '
            f'r_cost (pJ)/bit: {r_cost / bw}, w_cost ('
            f'pJ)/bit: {w_cost / bw}')
    exit()
